...
export default async function SettingsPage() {
  ...
  return <main className="stack"><div><div className="eyebrow">Regras da operação</div><h1>Configurações</h1></div>
    ...
    {isOrganizationManager(profile.role) ? <div className="card"><SettingsForm mode={(settings?.commission_mode || 'fixed') as CommissionMode} value={Number(settings?.commission_value || 0)} duration={settings?.default_duration_min || 60}/></div> : <div className="notice">Somente proprietários e administradores alteram comissão e duração padrão.</div>}
  </main>
}
