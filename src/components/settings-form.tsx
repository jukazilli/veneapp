...
export function SettingsForm({ mode, value, duration }: { mode: CommissionMode; value: number; duration: number }) {
  ...
  return <form action={action} className="form">
    ...
    <div className="field"><label>Duração padrão</label><select className="select" name="default_duration_min" defaultValue={duration}>{[30,45,60,90,120].map(v => <option key={v} value={v}>{v} minutos</option>)}</select></div>
